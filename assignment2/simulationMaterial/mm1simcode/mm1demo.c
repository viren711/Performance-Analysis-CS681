...
#define Q_LIMIT 3000  /* Limit on queue length. */
#define BUSY      1  /* Mnemonics for server's being busy */
#define IDLE      0  /* and idle. */
#define MAXRUNS   50
#define MAXDELAYS 5000
int   next_event_type, num_custs_delayed, num_delays_required, num_events,
  num_in_q, server_status,num_of_repetitions;
float area_num_in_q, area_server_status, mean_interarrival, mean_service,
      sim_time, time_arrival[Q_LIMIT + 1], time_last_event, time_next_event[3],
      total_of_delays, alldelays[MAXRUNS][MAXDELAYS];
FILE  *infile, *outfile, *delayfile;
int demo, run=0;
...
main()  /* Main function. */
{
    ...
    /*fscanf(infile, "%ld %d %f %f %d", &seed, &stream,&mean_interarrival, 
    ...
    scanf("%d", &num_of_repetitions);
    for (run = 0; run < num_of_repetitions; run++) {
      printf("Run %d\n", run);
      scanf ("%ld %d", &seed, &stream);
...
/*      fprintf(delayfile, "Run %d done \n", i+1); */
    ...
    for (j=0; j < num_delays_required; j++) {
      for (i=0; i < num_of_repetitions; i++) 
        fprintf(delayfile, "%.3f ", alldelays[i][j]);
      fprintf(delayfile,"\n");
    }
...
int i;	  
	...
	if (server_status== IDLE) printf ("Server Idle  | "); else printf ("Server Busy    | ");
	if (num_in_q) {
		printf ("%d in queue. buffer  (Arrival Timestamps): ", num_in_q);
	     	for (i = 1; i <= num_in_q; i++) printf("%.5f ", time_arrival[i]);
	}
	else
	 	printf("Queue Empty ");

	printf (" |  Next Arrival: %.5f  | Next Departure: ", time_next_event[1]);
	if (time_next_event[2] >= 1.0e+29) printf ("inf"); else printf ("%.2f", time_next_event[2]);
     	printf("\n\n");
}
